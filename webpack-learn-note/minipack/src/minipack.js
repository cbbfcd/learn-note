 ...
 * 
 * 我个人觉得这是前端工程化时代到来的标志。
 * 传统的通过一些 script 标签引入代码，然后再由浏览器解析执行的方式正在被革新。这种革新并不是说就不再是这样执行了或者脱离了以前的技术，
 * 实际上更像是代码书写方式、工程构建方式等方面的革新，比如通过打包工具打出 bundle 文件再由插件自动引入到浏览器解析执行是目前主流的方式。
 * 这一革新的源头就是模块系统等一系列新的技术、规范、思想的出现。
 * 对于打包工具，你只需要提供一个入口，工具就会自行建立依赖图进行分析，通过入口文件找到其依赖，然后再找到依赖的依赖...最后,
 * 把这些打包进一个 bundle 中。
 * 模块系统的诞生为诸如 webpack 之类的打包工具提供了成长环境，所以学习打包工具的基本原理是非常重要的。
 */

// 依赖
const fs = require('fs');
const path = require('path');
const babylon = require('babylon'); // babel 的解析器
const traverse = require('babel-traverse').default; // babel 遍历AST的工具函数
const {transformFromAst} = require('babel-core'); // babel 将AST转码的工具函数
...
// 该函数的作用就是提取文件内容，然后用 babylon 编译成 AST
// 通过遍历 AST，用 visitor 去访问所有的 import 节点，然后提取出 import 的内容存在一个队列
// 最后为了浏览器兼容性，输出编译后的代码和这个依赖的队列。如果对编译原理这块不熟悉的可以看看 babel 的文档
...
function createGraph(entry) {
  ...
  for (const asset of queue) {
    ...
    asset.dependencies.forEach(relativePath => {
      ...
      asset.mapping[relativePath] = child.id; // 相当于是存储依赖关系
